...
export abstract class ProtocolParser {
  ...
  async calcBadDebt(currTime: number): Promise<ParserResult> {
    ...
    let tvl2: number[] = [];  // Initialize an array to hold individual user net values

    for (const [user, data] of Object.entries(this.users)) {
      
      const userValue = ComputeUserValue(data, this.prices);

      // Round netValueUsd to 2 decimal places
      userValue.netValueUsd = parseFloat(userValue.netValueUsd.toFixed(2));
      ...
      tvl2.push(userValue.netValueUsd);  // Store individual user net values in the array

      if (user.toLowerCase() === '0xaacf2eb915a55dc1f562850061b7d86a3f18ef36') {
        console.log(`User data for 0xAaCF2eb915A55dc1f562850061b7D86a3f18EF36:`, userValue);
      }
      ...
      // Write the tvl2 array to a text file
      fs.writeFileSync('tvl2.txt', tvl2.join('\n'), 'utf8');
      console.log(`${this.runnerName}: tvl2 array saved to tvl2.txt`);
      
       // Calculate the sum of the tvl2 array
      const tvl2Sum = tvl2.reduce((acc, val) => acc + val, 0);
      console.log(`${this.runnerName}: Sum of tvl2 array: `, tvl2Sum);
    ...
    console.log(`${this.runnerName}: tvl: ${this.tvl}`);
    console.log(`${this.runnerName}: supply: ${totalCollateral}`);
    ...
    // Log the tvl2 array
    console.log(`${this.runnerName}: tvl2 array: `, tvl2);
...
function appendToCSV(userValue: UserValue): void {
  const filePath = path.resolve(process.cwd(), 'userValues.csv');
  const csvLine = `${userValue.collateralUsd},${userValue.debtUsd},${userValue.netValueUsd}\n`;

  try {
        fs.appendFileSync(filePath, csvLine);
        console.log('Successfully appended data to CSV');
    } catch (err) {
